...
var bytecode = require('./bytecode'),
    onByteCode = require('./on-bytecode');
...
function sectionFromHeap (lib, HEAP, cursor, optr) {
    ...
    var body = [];
    var i, j, index, len, count, nameHash,
        name, localCount, bodySize,
        sameTypeLocalCount, valueType, bodyLast;
    ...
    if (nameHash == 0x7c70da68) { // code
        ...
        for (j = 0; j < count; j++) {
            ...
            body.push({
                kind: 'func',
                body: onByteCode(HEAP, cursor, bodySize, optr)
            });
            cursor += bodySize;
        }
    } else {
